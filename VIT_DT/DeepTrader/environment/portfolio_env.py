...
class DataGenerator():
    ...
    def _step(self, update, trading_len_upd):
        #print("Cursor:",self.cursor)
        self.trade_len = trading_len_upd #a
        ...
        #print("End:",self.cursor, future_ror.shape, trade_masks.shape)
    ...
    def reset(self, start_point=None):
        ...
        if start_point is not None:
            ...
            cut_index = min(self.batch_size, len(self.tmp_order))
            self.cursor = self.tmp_order[:cut_index]
            self.tmp_order = self.tmp_order[cut_index:]

        #print("Before Reset - SC:", self.cursor)
        obs, market_obs, future_return, past_return = self._get_data()
        obs_masks, future_return_masks = self._get_masks(obs, future_return)
        trade_masks = np.logical_or(obs_masks, future_return_masks)
        obs = self._fillna(obs, obs_masks)
        ...
        #print("AFter Reset - SC:", self.cursor)
    ...
    def _get_data(self):
        ...
        for i, idx in enumerate(self.cursor):
            ...
            for j in range(self.assets_features):
                assets_states[i, :, :, j] = self.__assets_data[:, idx - self.window_len + 1: idx + 1, j]
    ...
    def step(self, action, p, simulation=False): # gets every tradinglength 252 D -> 10 times
        ...
        for i in range(1, self.trade_len+1):
          if simulation:
            ...
            if(i==self.trade_len):
              update = True
            obs, obs_normed, market_obs, market_obs_normed, future_ror, future_p, trade_masks, done1 = self.src._step(update, 1)
