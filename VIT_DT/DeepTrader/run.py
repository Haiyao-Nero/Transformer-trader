...
def run(func_args):
    ...
    if func_args.market == 'DJIA_v1':
        ...
        # test_idx = 7328
        val_idx = 2900
        test_idx = 2900    # 2649 - year; 2900 - days; 2880 - M; 2859 - 2M
        allow_short = True
    ...
    supports = [A]
    # RLActor(supports, func_args).to(func_args.device)
    actor = RLActor(supports, func_args).to(func_args.device)
    agent = RLAgent(env, actor, func_args)
    if func_args.resume:
        agent.actor = torch.load(func_args.resume_model_path).to(func_args.device)
    ...
    if func_args.mode == 'train':
        try:
            ...
            for epoch in range(0,func_args.epochs):
                ...
                writer.add_scalar('Eval/Return', agent_wealth[0,-1], global_step=epoch)
                writer.add_scalar('Eval/APR', metrics['APR'], global_step=epoch)
                writer.add_scalar('Eval/MDD', metrics['MDD'], global_step=epoch)
                writer.add_scalar('Eval/AVOL', metrics['AVOL'], global_step=epoch)
                writer.add_scalar('Eval/ASR', metrics['ASR'], global_step=epoch)
                writer.add_scalar('Eval/SoR', metrics['DDR'], global_step=epoch)
                writer.add_scalar('Eval/CR', metrics['CR'], global_step=epoch)
                
                if metrics['CR'] > max_cr:
                    print('New Best CR Policy!!!!')
                    max_cr = metrics['CR']
                    torch.save(actor, os.path.join(model_save_dir, 'best_cr-'+str(epoch)+'.pkl'))
                logger.warning('Eval Result: after training %d round, max wealth: %.4f, min wealth: %.4f,'
                ...
                metrics = calculate_metrics(agent_wealth, func_args.trade_mode)
                writer.add_scalar('Test/Return', agent_wealth[0,-1], global_step=epoch)
                ...
                logger.warning('Test Result: after training %d round, max wealth: %.4f, min wealth: %.4f,'
                                ' avg wealth: %.4f, final wealth: %.4f, ARR: %.3f%%, ASR: %.3f, AVol" %.3f,'
                                'MDD: %.2f%%, CR: %.3f, DDR: %.3f'
                                % (
                                    epoch, max(agent_wealth[0]), min(agent_wealth[0]), np.mean(agent_wealth),
    ...
    elif func_args.mode == "test":
        agent_wealth = agent.test()
        print(agent_wealth.shape)
  
        with open("VIT_AW_252_Daily.csv", "w") as f:
          data_row = ",".join(str(value) for value in agent_wealth.flatten())
          f.write(data_row + "\n")

        # calculating metrics
        metrics = calculate_metrics(agent_wealth, func_args.trade_mode)
        print('ARR', 100*metrics['APR'])
        ...
        print(max(agent_wealth[0]), min(agent_wealth[0]))
        plt.plot(agent_wealth.T)
        plt.savefig('VIT_AW_252_Daily.png')
        print("over")
